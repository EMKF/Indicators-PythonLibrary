...
def time_series(df_in, var_lst=None, strata_dic=None, show=True, save_path=None, title=None, to_index=False,
         recessions=False, filter=False, start_year=None, end_year=None, day_marker=None, frequency='yearly'):
    """
    var_lst: list or dict
        If dict, the keys are the column names from the dataframe and the values are corresponding descriptions. If
        a list, then the column names are used as the descriptions. These are the covariates from the dataframe you
    ...
    if frequency == 'weekly':  # if weekly
        lamb = 1600  # hmmm
        # lamb = 45697600
        df_in = df_in.assign(time=lambda x: pd.to_datetime(x['time'].astype(str)))
        offset = {'weeks': 1}
    elif frequency == 'monthly':  # if monthly
        lamb = 129600
        df_in = df_in._obj.assign(time=lambda x: pd.to_datetime(x['time'].astype(str)))
        offset = {'months': 1}
    elif frequency == 'quarterly':  # if quarterly
        lamb = 1600
        df_in = df_in._obj.assign(time=lambda x: pd.to_datetime(x['time'].astype(str)))
        offset = {'months': 3}
    else:  # if yearly
